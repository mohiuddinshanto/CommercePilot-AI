...
import { ChevronLeft, ChevronRight, Menu, Store, X } from "lucide-react";
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-5 left-5 z-40 rounded-full bg-blue-600 p-3 text-white shadow-lg md:hidden"
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex h-screen -translate-x-full flex-col border-r border-gray-200 bg-white transition-all duration-300 md:static md:translate-x-0",
      ...
      >
        <div className="flex h-16 items-center justify-between border-b border-gray-200 px-4">
          ...
          {!collapsed && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-bold text-gray-900">CommercePilot</span>
            </div>
          )}
        ...
        <nav className="flex-1 space-y-1 px-2 py-4">
          {visibleItems.map((item: NavItem) => {
            ...
            const label = T(navLabelKey[item.label] || item.label);
            return (
              <Link
                key={item.href}
                href={item.href}
